...
layui.config({
		...
		function renderDate(data,curr){
			...
			if(dataList.length != 0){
				for(var i=0;i<dataList.length;i++){
			    	...
			    	+  '<td>'+dataList[i].provinceId+'</td>'
			    	+  '<td>'+dataList[i].cityId+'</td>'
			    	+  '<td>'+dataList[i].areaId+'</td>'
			    	+  '<td>'+dataList[i].defaultStatus+'</td>'
			    	+  '<td>'+dataList[i].shipperId+'</td>'
			    	+  '<td>'+dataList[i].transporterId+'</td>'
	...
	$("body").on("click",".data_detail",function(){  
		...
		var index = layui.layer.open({
			title : "Detail",
			type : 2,
			content : "/goodsSource/toDetail?id="+_this.attr("data-id"),
