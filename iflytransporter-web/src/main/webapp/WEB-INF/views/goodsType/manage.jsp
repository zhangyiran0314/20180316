...
layui.config({
	...
	var form = layui.form,
		...
		table.render({
		    ...
		    ,height: 315
		    ,id:"layui_table_id"
		    ,url: '<%=request.getContextPath()%>/goodsType/queryPage' //数据接口
		    ,page: true //开启分页
		    ,cols: [[ //表头
         ...
         table.on('tool(table_filter)', function(obj){ //注：tool是工具条事件名，table_filter是table原始容器的属性 lay-filter="对应的值"  
		    ...
		    if(obj.event === 'del'){
		      layer.confirm('确认永久删除?', function(index){
		    	 $.ajax({  
		                url: "<%=request.getContextPath()%>/goodsType/delete",
		                type: "post",
		                dataType:"json",
		                data:{id:data.id},
		    	 ...
		    	 layui.use('layer', function(){
					  ...
					  layer.open({
						  ...
						  fixed: false, //不固定
						  maxmin: true,
						  content: "<%=request.getContextPath()%>/goodsType/toEdit?id="+data.id,
		    	 ...
		    	 layui.use('layer', function(){
					  ...
					  layer.open({
						  ...
						  fixed: false, //不固定
						  maxmin: true,
						  content: "<%=request.getContextPath()%>/goodsType/toDetail?id="+data.id,
						  /* success: function (layero, index) {  
			                    // 获取子页面的iframe  
			                    var iframe = window['layui-layer-iframe' + index];  
			                    // 向子页面的全局函数child传参  
         ...
         $(".add_btn").click(function(){
 			layui.use('layer', function(){
 			  ...
 			  layer.open({
 				  ...
 				  fixed: false, //不固定
 				  maxmin: true,
 				  content: '<%=request.getContextPath()%>/goodsType/toAdd'
 				}); 
 			}); 
 		})
         //日期用法  
