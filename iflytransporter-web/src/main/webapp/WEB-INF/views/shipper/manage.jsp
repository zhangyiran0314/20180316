...
layui.config({
	...
	function getDatas(){
	    	$.ajax({
	    		type:"GET",
	    		url:"<%=request.getContextPath()%>/shipper/queryPage",
	    		dataType:"json",
			...
			if(dataList.length != 0){
				for(var i=0;i<dataList.length;i++){
			    	...
			    	if(dataList[i].status==0){
			    		dataHtml +=  '<td>UserRegister</td>';
			    	}else if(dataList[i].status== 1){
			    		dataHtml +=  '<td>UserIdentify</td>';
			    	}else{
			    		dataHtml +=  '<td>UserEnable</td>';
			    	}
			    	dataHtml += '<td>'+dataList[i].lastLoginDate+'</td>'
	...
	$("body").on("click",".data_detail",function(){  
		...
		var index = layui.layer.open({
			title : "Detail",
			type : 2,
			content : "<%=request.getContextPath()%>/shipper/toDetail?id="+_this.attr("data-id"),
