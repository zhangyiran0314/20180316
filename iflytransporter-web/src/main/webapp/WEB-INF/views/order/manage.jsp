...
layui.config({
		...
		function renderDate(data,curr){
			...
			if(dataList.length != 0){
				for(var i=0;i<dataList.length;i++){
			    	...
			    	+  '<td>'+dataList[i].departure+'</td>'
			    	+  '<td>'+dataList[i].destination+'</td>'
			    	+  '<td>'+dataList[i].carTypeValue+'</td>'
			    	+  '<td>'+dataList[i].goodsTypeValue+'</td>'
			    	+  '<td>'+dataList[i].goodsUnitValue+'</td>'
			    	+  '<td>'+dataList[i].deliverDate+'</td>'
			    	+  '<td>'+dataList[i].remark+'</td>'
			    	+  '<td>'+dataList[i].repeatFlag+'</td>'
	...
	$("body").on("click",".data_detail",function(){  
		...
		var index = layui.layer.open({
			title : "Detail",
			type : 2,
			content : "/order/toDetail?id="+_this.attr("data-id"),
