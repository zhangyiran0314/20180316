...
layui.config({
	...
	var form = layui.form,
		...
		table.render({
		    ...
		    ,height: 500
		    ,id:"layui_table_id"
		    ,url: '<%=request.getContextPath()%>/order/queryPage' //数据接口
		    ,page: true //开启分页
		    ,cols: [[ //表头
		      ...
		      layer.confirm('确认永久删除?', function(index){
		    	 ...
		    	 layui.use('layer', function(){
					  ...
					  layer.open({
						  ...
						  fixed: false, //不固定
						  maxmin: true,
						  content: "<%=request.getContextPath()%>/order/toEdit?id="+data.id,
		    	 ...
		    	 layui.use('layer', function(){
					  ...
					  layer.open({
						  ...
						  fixed: false, //不固定
						  maxmin: true,
						  content: "<%=request.getContextPath()%>/order/toDetail?id="+data.id,
						  /* success: function (layero, index) {  
			                    // 获取子页面的iframe  
			                    var iframe = window['layui-layer-iframe' + index];  
			                    // 向子页面的全局函数child传参  
		    	 ...
		    	 layui.use('layer', function(){
					  ...
					  layer.open({
						  ...
						  fixed: false, //不固定
						  maxmin: true,
						  content: "<%=request.getContextPath()%>/order/toEditAuth?id="+data.id,
						  /* success: function (layero, index) {  
			                    // 获取子页面的iframe  
			                    var iframe = window['layui-layer-iframe' + index];  
			                    // 向子页面的全局函数child传参  
         ...
         $(".add_btn").click(function(){
 			layui.use('layer', function(){
 			  ...
 			  layer.open({
 				  ...
 				  fixed: false, //不固定
 				  maxmin: true,
 				  content: '<%=request.getContextPath()%>/order/toAdd'
 				}); 
 			}); 
 		})
         //日期用法  
